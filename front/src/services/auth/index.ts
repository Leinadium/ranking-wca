...
import type {
	...
	APIUpdateUserInformationsResponse
...
export const authService = {
	...
	): Promise<APIGetUserInformationsResponse> {
		return await HTTPService.getAsync({
			...args,
			abortSignalKey: ABORT_SIGNALS_KEYS.AUTH_CALLBACK,
			url: `${API_URL}${API_ENDPOINTS.AUTH_CALLBACK}?code=${args?.code}`,
			errorMessage:
				'Não foi possível carregar dados pessoais do usuário. Por favor, tente novamente.'
		});
	},

	async updateUserInformations(
		args?: UpdateUserInformationsArgs
	): Promise<APIUpdateUserInformationsResponse> {
		return await HTTPService.postAsync({
			...
			errorMessage:
				'Não foi possível atualizar dados pessoais do usuário. Por favor, tente novamente.'
		});
	}
};
