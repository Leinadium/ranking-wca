...
export const personService = {
	...
	async getCurrentRecords(
		args: GetPersonCurrentRecordsArgs
	): Promise<APIPersonCurrentRecordsResponse> {
		return await HTTPService.getAsync({
			...args,
			abortSignalKey: ABORT_SIGNALS_KEYS.PERSON_CURRENT_RECORDS,
			url: `${API_URL}${API_ENDPOINTS.PERSON_CURRENT_RECORDS}/${args.wcaId}`,
			errorMessage:
				'Não foi possível carregar dados de recordes atuais da pessoa. Por favor, tente novamente.'
		});
	},

	async getRankingByMode(
		args: GetPersonRankingByModeArgs
	): Promise<APIPersonRankingByModeResponse> {
		return await HTTPService.getAsync({
			...args,
			...
			errorMessage:
				'Não foi possível carregar dados de recordes detalhados da pessoa. Por favor, tente novamente.'
		});
	},
