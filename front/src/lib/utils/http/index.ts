...
export default class HTTPService {
	static async _getFormattedReponse(response: Response): Promise<any> {
	...
	static async _requestDataAsync<T>(args: RequestDataArgs): Promise<CustomResponse<T>> {
		...
		const requestOptions = {
			method: args.method,
			...(args.body && {
				body: checkIsPlainObject(args.body) ? JSON.stringify(args.body) : args.body
			}),
			...(args.abortSignalKey && {
				signal: AbortSignalsManager.abortAndGetSignal(args.abortSignalKey)
			})
		};
		...
		if (!responsePromise?.ok || responsePromise?.status === 404) {
			throw new CustomError(
				isJSONResponse ? responseData?.error : responseData,
				responsePromise?.status
			);
		}
		...
		if (
			args.successMessage &&
			responsePromise?.ok &&
			responsePromise?.status >= 200 &&
			responsePromise?.status < 300
		) {
