<script lang="ts">
	...
	import {
		loadPersonCurrentRecords,
		loadPersonImage,
		loadPersonInfo,
		loadPersonRankingByModeRecords
	} from '../../../../viewModels/person';
	...
	import type {
		CurrentRecordsTableData,
		RankingByModeTableData,
		RankingByModeTableFiltersProps
	} from './types';
	...
	function handleSortChange(
		newDirection: TableSortDirectionOptions,
		column: keyof PersonEventResultViewModel | string
	) {
	...
	$effect(() => {
		rankingByModeTableData.itemsPerPage =
			$personStore.rankings?.[tableFilters.competitionMode]?.length;
	});
	...
	$effect(() => {
		rankingByModeTableData.totalItems =
			$personStore.rankings?.[tableFilters.competitionMode]?.length;
	});
		...
		{#if $personStore.stateId}
			<GridItem justifyContent={'flex-start'} gap={1}>
				...
				<Typography type={'bodyOne'} color={'NEUTRAL_DARK_1'}
					>{STATE_NAMES[$personStore.stateId]}</Typography
				>
			</GridItem>
		{/if}
	</GridItem>
		...
		<GridItem justifyContent={'center'} alignItems={'center'} gap={4}>
			<div class="portrait">
				<img
					...
					src={$personStore.imageUrl || DEFAULT_PERSON_AVATAR_IMAGE_SRC}
					alt="Imagem do {$personStore.name}"
				/>
			</div>
			...
			<GridItem direction={'COLUMN'} alignItems={'flex-start'} gap={3}>
				...
				<GridItem alignItems={'flex-start'} gap={1} isFullWidth>
					...
					<Tag
						color={'SECONDARY'}
						text={$personStore.totalCompetitionsCount || DEFAULT_NO_DATA_VALUE}
						isFullWidth
					/>
				</GridItem>
				...
				<GridItem alignItems={'flex-start'} gap={1} isFullWidth>
					...
					<Tag
						color={'SECONDARY'}
						text={$personStore.stateCompetitionsCount || DEFAULT_NO_DATA_VALUE}
						isFullWidth
					/>
				</GridItem>
			</GridItem>
					...
					{#if row.eventId}
						<TableRow>
							...
							{#key row.rankingSingle}
								<TableCell isHighlighted={checkShouldHighlightPosition(row.rankingSingle)}>
									{formatValueAsInt(row.rankingSingle)}
								</TableCell>
							{/key}
							{#key row.rankingSingle}
								<TableCell isHighlighted={checkShouldHighlightPosition(row.rankingAverage)}>
		...
		<TableBase>
			...
			<TableBody>
				{#each rankingByModeTableData.paginatedData as row}
					...
					<TableRow>
						{#key row.ranking}
							<TableCell isHighlighted={checkShouldHighlightPosition(row.ranking)}>
								{row.ranking}
							</TableCell>
						{/key}
						<TableCell>
